...
public class Ventana {
    ...
    public Ventana() {
        ...
        // ================= DATOS QUEMADOS =================
        personajes.anadir(new PersonajeAnime(1, "Goku", "Dragon Ball", 9000, "Kamehameha"));
        personajes.anadir(new PersonajeAnime(2, "Vegeta", "Dragon Ball", 8800, "Final Flash"));
        personajes.anadir(new PersonajeAnime(3, "Naruto", "Naruto", 8500, "Rasengan"));
        personajes.anadir(new PersonajeAnime(4, "Sasuke", "Naruto", 8400, "Chidori"));
        personajes.anadir(new PersonajeAnime(5, "Luffy", "One Piece", 8700, "Gear Fifth"));

        // Mostrar lista inicial ordenada
        txtResumen.setText("");
        List<PersonajeAnime> demoOrdenados = personajes.listarPersonajes();
        Collections.sort(demoOrdenados);
        for (PersonajeAnime p : demoOrdenados) {
            txtResumen.append(p.toString() + "\n");
        }

        // Ejemplo de ajuste de poder usando la misma lógica que los botones
        PersonajeAnime gokuBuff = new PersonajeAnime(100, "Goku", "Dragon Ball", 9000, "Kamehameha");
        if (gokuBuff.toString().contains("Dragon Ball")) {
            gokuBuff.setNivelPoder(9000 + 9000 * 0.2); // +20%
        }
        personajes.anadir(gokuBuff);

        PersonajeAnime narutoBuff = new PersonajeAnime(101, "Naruto", "Naruto", 8500, "Rasengan");
        if (narutoBuff.toString().contains("Naruto")) {
            narutoBuff.setNivelPoder(8500 + 8500 * 0.15); // +15%
        }
        personajes.anadir(narutoBuff);

        // Mostrar ajustes en el área de texto correspondiente
        txtAjustes.setText("");
        List<PersonajeAnime> demoAjustados = personajes.listarPersonajes();
        Collections.sort(demoAjustados);
        for (PersonajeAnime p : demoAjustados) {
